...
//    private CardReaderClientSpec cardReaderClient = new MemoryCardReaderClient();
//    private BankApiClientSpec bankApiClient = new MemoryBankApiClient();
//    private CacheBinClientSpec cacheBinClient = new CacheBinTestClient();
    ...
    @Test
    void readCard() throws CardReadException {
        Card actual = MemoryCardReaderClient.registerNewCard("sungmin");
        Card expect = atmService.readCard();
    ...
    void getAccounts(String name, Card card, List<Account> expect) {
        List<Account> actual = atmService.getAccounts(card);
    ...
    void validatePIN(String name, Long settingPin, Long tryPin, Boolean expect) {
        ...
        Boolean actual = atmService.validatePIN(card, tryPin);
        assertThat(actual).isEqualTo(expect);
    }
    ...
    @ParameterizedTest(name = "[{index}] {0}")
    @MethodSource
    void deposit(String name, Account account, Long dollar, Long expect) throws CacheBinException, BankLossDepositException, BankApiException {
        atmService.deposit(account, dollar);
    ...
    @ParameterizedTest(name = "[{index}] {0}")
    @MethodSource
    void withdraw(String name, Account account, Long dollar, Long balanceExpect, Boolean isSuccessExpect) throws CacheBinException, UserLossWithdrawException, BankApiException {
        Boolean isSuccessActual = atmService.withdraw(account, dollar);
        Long actual = account.getBalance();
